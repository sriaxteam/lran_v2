...
def load_citizen_voice_xlsx(date_str):
    ...
    try:
        ...
        df = df[df["channel"].isin(["naver", "blog", "cafe", "youtube", "local", "daangn"])].copy()
        if df.empty:
            return None
...
# ═══════════════════════════════════════════════════════════
_raw_critiques = minseang.get("검증_포인트", [])
if _raw_critiques and isinstance(_raw_critiques, list) and isinstance(_raw_critiques[0], dict):
    critiques = [(c.get("title",""), c.get("body","")) for c in _raw_critiques]
else:
    critiques = [
